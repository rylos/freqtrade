#!/usr/bin/env python3
"""Watchdog del bot freqtrade RyLoS (T4G, HYPE/USDT:USDT su bybit) su amazon.
...
HEARTBEAT_RE = re.compile(
    r"Bot heartbeat\. PID=(\d+), version='([^']*)', state='([^']*)'"
)
...
def log(msg: str) -> None:
    ...
    with open(BASE / "freqtrade_watchdog.log", "a") as f:
        f.write(f"{stamp} {msg}\n")
...
def bot_pid() -> str | None:
    r = subprocess.run(
        ["pgrep", "-f", "bin/freqtrade"], capture_output=True, text=True
    )
...
def try_restart(state: dict) -> list[str]:
    ...
    now = time.time()
    recent = [
        t for t in state.get("restarts", []) if now - t < RESTART_WINDOW_MIN * 60
    ]
...
def main() -> None:
    ...
    for line in reversed(lines):
        ...
        if m:
            ...
            hb_pid, version, hb_state = m.groups()
    ...
    ok = not problems
    head = "freqtrade RyLoS T4G (amazon) — " + ("OK" if ok else "ANOMALIA")
