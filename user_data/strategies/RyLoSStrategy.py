...
class RyLoSStrategy(IStrategy):
    ...
    first_order_pct = DecimalParameter(0.005, 0.10, default=0.005, space="buy", optimize=True)
    dca_distance = DecimalParameter(0.005, 0.05, default=0.029, space="buy", optimize=True)
    dca_multiplier = DecimalParameter(1.5, 3.0, default=2.944, space="buy", optimize=True)

    # Parametri entry ottimizzabili - Multi-Oscillator Oversold
    rsi_oversold_threshold = DecimalParameter(25, 40, default=25.78, space="buy", optimize=True)
    bb_oversold_threshold = DecimalParameter(0.1, 0.3, default=0.236, space="buy", optimize=True)
    atr_oversold_multiplier = DecimalParameter(0.5, 2.0, default=1.979, space="buy", optimize=True)
    ...
    williams_oversold_threshold = DecimalParameter(
        -90, -70, default=-72.878, space="buy", optimize=True
    )
    min_oversold_count = IntParameter(2, 5, default=2, space="buy", optimize=True)

    # Emergency DCA (prima della liquidazione)
    emergency_dca_threshold = DecimalParameter(
        -0.18, -0.10, default=-0.166, space="buy", optimize=True
    )
    emergency_critical_multiplier = DecimalParameter(
        1.05, 2.0, default=1.863, space="buy", optimize=True
    )

    # Parametri per exit ottimizzabili - Multi-Oscillator Overbought
    rsi_overbought_threshold = DecimalParameter(60, 85, default=75.81, space="sell", optimize=True)
    bb_overbought_threshold = DecimalParameter(0.7, 0.9, default=0.9, space="sell", optimize=True)
    atr_overbought_multiplier = DecimalParameter(
        0.5, 2.0, default=0.741, space="sell", optimize=True
    )
    ...
    williams_overbought_threshold = DecimalParameter(
        -30, -10, default=-10.162, space="sell", optimize=True
    )
    min_overbought_count = IntParameter(2, 5, default=4, space="sell", optimize=True)
    min_profit_for_overbought_exit = DecimalParameter(
        0.01, 0.10, default=0.011, space="sell", optimize=True
    )

    # Last DCA Profit Skimming
    last_dca_profit_threshold = DecimalParameter(
        0.01, 0.10, default=0.089, space="sell", optimize=True
    )

    # Auto-Reduce per over-exposure
    auto_reduce_threshold = DecimalParameter(1.0, 1.1, default=1.053, space="sell", optimize=True)
    auto_reduce_critical_threshold = DecimalParameter(
        1.06, 1.15, default=1.074, space="sell", optimize=True
    )

    # Trailing stop
    trailing_stop = True
    trailing_stop_positive = 0.131
    trailing_stop_positive_offset = 0.222
        ...
        if (
            ...
            if current_loss_from_avg <= critical_threshold and current_rate < last_order_price:
                ...
                if emergency_stake >= min_stake:
                    loss_pct = abs(current_loss_from_avg * 100)
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        global_limit = total_balance * 4  # Limite globale con leva 4x
        current_global_exposure = self.get_total_position_value()
        ...
        if exposure_ratio > self.auto_reduce_threshold.value:
            ...
            current_position_exposure = trade.stake_amount * 4
            position_ratio = (
            ...
            reduce_amount = reduce_exposure / 4  # Diviso leva per ottenere stake amount
