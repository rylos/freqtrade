...
class RyLoSStrategy(IStrategy):
    ...
    minimal_roi = {
        "0": 10.0  # 1000% - praticamente disabilitato
    }

    # Parametri ottimizzabili
    first_order_pct = DecimalParameter(0.005, 0.10, default=0.053, space='buy', optimize=True)
    dca_distance = DecimalParameter(0.002, 0.05, default=0.022, space='buy', optimize=True)
    dca_multiplier = DecimalParameter(1.5, 3.0, default=2.018, space='buy', optimize=True)

    # Parametri entry ottimizzabili - Multi-Oscillator Oversold
    rsi_oversold_threshold = DecimalParameter(25, 40, default=26.221, space='buy', optimize=True)
    bb_oversold_threshold = DecimalParameter(0.1, 0.3, default=0.105, space='buy', optimize=True)
    atr_oversold_multiplier = DecimalParameter(0.5, 2.0, default=0.758, space='buy', optimize=True)
    macd_oversold_threshold = DecimalParameter(-0.01, -0.001, default=-0.006, space='buy', optimize=True)
    williams_oversold_threshold = DecimalParameter(-90, -70, default=-79.415, space='buy', optimize=True)
    min_oversold_count = IntParameter(2, 5, default=3, space='buy', optimize=True)

    # Emergency DCA (prima della liquidazione)
    emergency_dca_threshold = DecimalParameter(-0.18, -0.10, default=-0.17, space='buy', optimize=True)
    emergency_critical_multiplier = DecimalParameter(1.05, 2.0, default=1.59, space='buy', optimize=True)

    # Parametri per exit ottimizzabili - Multi-Oscillator Overbought
    rsi_overbought_threshold = DecimalParameter(60, 85, default=71.587, space='sell', optimize=True)
    bb_overbought_threshold = DecimalParameter(0.7, 0.9, default=0.858, space='sell', optimize=True)
    atr_overbought_multiplier = DecimalParameter(0.5, 2.0, default=0.631, space='sell', optimize=True)
    macd_overbought_threshold = DecimalParameter(0.001, 0.01, default=0.007, space='sell', optimize=True)
    williams_overbought_threshold = DecimalParameter(-30, -10, default=-11.154, space='sell', optimize=True)
    min_overbought_count = IntParameter(2, 5, default=4, space='sell', optimize=True)
    min_profit_for_overbought_exit = DecimalParameter(0.01, 0.10, default=0.013, space='sell', optimize=True)

    # Last DCA Profit Skimming
    last_dca_profit_threshold = DecimalParameter(0.01, 0.10, default=0.04, space='sell', optimize=True)

    # Auto-Reduce per over-exposure
    auto_reduce_threshold = DecimalParameter(1.0, 1.1, default=1.024, space='sell', optimize=True)
    auto_reduce_critical_threshold = DecimalParameter(1.06, 1.15, default=1.146, space='sell', optimize=True)


    def leverage(self, pair: str, current_time, current_rate, proposed_leverage, max_leverage, entry_tag, side: str, **kwargs) -> float:
        return 5.0
    ...
    def custom_stake_amount(self, pair: str, current_time, current_rate: float, 
                            proposed_stake: float, min_stake, max_stake: float, leverage: float, 
    ...
    def calculate_max_orders(self, total_balance: float) -> int:
        ...
        for i in range(20):  # Limite di sicurezza
            if i == 0:
                ...
                stake_pct = self.first_order_pct.value * (self.dca_multiplier.value ** (i-1))
        ...
        return max(1, order_count - 1)  # -1 perché primo ordine non conta come adjustment

    def adjust_trade_position(self, trade: Trade, current_time, current_rate: float, current_profit: float, min_stake, max_stake: float, current_entry_rate: float, current_exit_rate: float, current_entry_profit: float, current_exit_profit: float, **kwargs) -> float:
        total_balance = self.wallets.get_total_stake_amount()
        max_open_trades = self.config.get('max_open_trades', 1)
        ...
        if (current_loss_from_avg <= self.emergency_dca_threshold.value and
            trade.nr_of_successful_entries < max_orders):  # Rispetta max_orders

            # Soglia critica = emergency_threshold * multiplier
            critical_threshold = self.emergency_dca_threshold.value * self.emergency_critical_multiplier.value

            if (current_loss_from_avg <= critical_threshold and
                current_rate < last_order_price):

                # Calcola stake come un DCA normale
                next_dca_order = trade.nr_of_successful_entries + 1
                stake_pct = self.first_order_pct.value * (self.dca_multiplier.value ** (next_dca_order - 1))
        ...
        if trade.orders:
            ...
            if (price_distance < self.dca_distance.value or 
                current_rate >= last_order_rate):
                return None
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        rsi_oversold = (dataframe['rsi'] < self.rsi_oversold_threshold.value).fillna(False).astype(int)
        bb_oversold = (dataframe['bb_percent'] < self.bb_oversold_threshold.value).fillna(False).astype(int)

        # ATR oversold: prezzo vicino al low recente (ATR-based)
        atr_low_threshold = dataframe['low'].rolling(14).min() + (dataframe['atr'] * self.atr_oversold_multiplier.value)
        atr_oversold = (dataframe['close'] < atr_low_threshold).fillna(False).astype(int)

        # MACD oversold: MACD sotto soglia negativa
        macd_oversold = (dataframe['macd'] < self.macd_oversold_threshold.value).fillna(False).astype(int)

        # Williams %R oversold
        williams_oversold = (dataframe['williams_r'] < self.williams_oversold_threshold.value).fillna(False).astype(int)

        oversold_count = rsi_oversold + bb_oversold + atr_oversold + macd_oversold + williams_oversold

        # Crea tag parlanti per indicare quali indicatori hanno scatenato l'entry
        dataframe['entry_tag'] = ''

        entry_condition = (oversold_count >= self.min_oversold_count.value) & (dataframe['close'] < dataframe['open'])

        for i in range(len(dataframe)):
            if entry_condition.iloc[i]:
                indicators = []
                if rsi_oversold.iloc[i]: indicators.append('rsi')
                if bb_oversold.iloc[i]: indicators.append('bb')
                if atr_oversold.iloc[i]: indicators.append('atr')
                if macd_oversold.iloc[i]: indicators.append('macd')
                if williams_oversold.iloc[i]: indicators.append('wr')
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time, current_rate: float, current_profit: float, **kwargs):
        ...
        if exposure_ratio > self.auto_reduce_threshold.value:
            ...
            if trade.nr_of_successful_entries >= max_orders and trade.orders:
                ...
                ultimo_dca_profittevole = current_rate > last_order_price * (1 + self.last_dca_profit_threshold.value)

            # Se siamo POCO over-exposed E ultimo DCA è profittevole
            if exposure_ratio < self.auto_reduce_critical_threshold.value and ultimo_dca_profittevole:
            ...
            position_ratio = current_position_exposure / current_global_exposure if current_global_exposure > 0 else 0
        ...
        if current_profit > self.min_profit_for_overbought_exit.value:
            ...
            if current_candle['close'] > (current_candle['high'] - current_candle['atr'] * self.atr_overbought_multiplier.value):
            ...
            if (overbought_count >= self.min_overbought_count.value and 
